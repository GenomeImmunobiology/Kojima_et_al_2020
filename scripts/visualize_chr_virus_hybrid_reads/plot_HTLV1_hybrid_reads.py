...
def plot(dir):
    try:
                ...
                if not r == read:
                    ...
                    if -1 * near_len < (chr_pos - chr_pos2) < near_len:
                        if not chr_strand == chr_strand2:
                            ...
                            min_dist.add(abs(chr_pos - chr_pos2))
        if len(breakpoints) == 0:
            info='%s\t%d\t%d\t%d\t%d\n' % (sample_id, len(mapped_rn), hybrid_n, len(mapped), len(breakpoints))
        else:
            min_dist=list(min_dist)
            min_dist=[ str(v) for v in min_dist ]
            info='%s\t%d\t%d\t%d\t%d;%s\n' % (sample_id, len(mapped_rn), hybrid_n, len(mapped), len(breakpoints), '|'.join(min_dist))
        out.append(info)
        print(info, end='')
        ...
        plt.savefig('%s/plot_%s_htlv_hybrid.pdf' % (dir, sample_id))
        plt.close()
    except:
...
with open('1kGP_htlv_hybrid_summary.txt', 'w') as outfile:
